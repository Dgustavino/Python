...
class Controller:
    ...
    def __init__(self):
        # init construct
        self._lista_estudiante = []  # creacion de lista para almacenar instancias del modelo estudiante
    ...
    def addStudent(self,estudiante): 
        ...
        self._lista_estudiante.append(estudiante)


    def updateView(self): 
        # despliego cantidad estudiantes existentes
        self.vista.mensaje("LISTA DE ESTUDIANTES: "+ str(len(self._lista_estudiante))) 
        # top message cada vez que se despliega el contenido de lista

        if len(self._lista_estudiante)==0: # si la lista esta vacia
            self.vista.mensaje("No se han ingresado datos aun!") # utilizo la vista para enviar mensaje

        else: # si len>0 
            for estudiante in self._lista_estudiante: # recorre items per iterable obj
    ...
    def agregar(self): 
        ...
        try:
            ...
            if userInput=='Y':
                datos_newStudent = input("Ingrese el codigo y el nombre separado por coma-> COD,Nombre Apellido [Sin espacios al inicio]: ").split(',') 
                # con split puedo capturar varios inputs del usuario
                estudiante_nuevo = Student(datos_newStudent[0], datos_newStudent[1]) 
